...
SHA=$(curl -s http://download.khulnasoft.com/releases/redis-${1}.tar.gz | shasum -a 256 | cut -f 1 -d' ')
ENTRY="hash redis-${1}.tar.gz sha256 $SHA http://download.khulnasoft.com/releases/redis-${1}.tar.gz"
